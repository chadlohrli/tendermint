...
LD_FLAGS = -X github.com/providenetwork/tendermint/version.TMCoreSemVer=$(VERSION)
BUILD_FLAGS = -mod=readonly -ldflags "$(LD_FLAGS)"
HTTPS_GIT := https://github.com/providenetwork/tendermint.git
...
draw_deps:
	...
	@goviz -i github.com/providenetwork/tendermint/cmd/tendermint -d 3 | dot -Tpng -o dependency-graph.png
.PHONY: draw_deps
...
format:
	...
	find . -name '*.go' -type f -not -path "*.git*"  -not -name '*.pb.go' -not -name '*pb_test.go' | xargs goimports -w -local github.com/providenetwork/tendermint
.PHONY: format
