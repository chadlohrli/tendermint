...
import (
	...
	"github.com/providenetwork/tendermint/internal/libs/tempfile"
...
type Validator struct {
	...
	Config        *config.PrivValidatorConfig `json:"-"`
...
func LoadOrGenValidator(cfg *config.PrivValidatorConfig, vaultRefreshToken string, vaultID uuid.UUID, vaultKeyID *uuid.UUID) (*Validator, error) {
	var v *Validator

	if tmos.FileExists(cfg.Key) {
		raw, err := ioutil.ReadFile(cfg.Key)
		if err != nil {
			return nil, err
		}
		...
		raw, err = ioutil.ReadFile(cfg.State)
		if err == nil {
			err = tmjson.Unmarshal(raw, &v.LastSignState)
			if err != nil {
				return nil, err
			}
			v.LastSignState.StatePath = cfg.State
		}
	} else {
		...
		val, err := ValidatorFactory(cfg, vaultRefreshToken, vaultID, privKey.VaultKeyID)
		if err != nil {
			return nil, err
		}
		v = val
...
func ValidatorFactory(cfg *config.PrivValidatorConfig, token string, vaultID, vaultKeyID uuid.UUID) (*Validator, error) {
	v := &Validator{
		Config:            cfg,
		VaultID:           vaultID,
...
func (v *Validator) GetPubKey(ctx context.Context) (crypto.PubKey, error) {
...
func (v *Validator) SignProposal(ctx context.Context, chainID string, proposal *tmproto.Proposal) error {
...
func (v *Validator) SignVote(ctx context.Context, chainID string, vote *tmproto.Vote) error {
...
func (v *Validator) Save() {
	...
	err = tempfile.WriteFileAtomic(v.Config.Key, jsonBytes, 0600)
	if err != nil {
		panic(err)
	}
}
...
func (lss *LastSignState) Save() {
	...
	err = tempfile.WriteFileAtomic(lss.StatePath, jsonBytes, 0600)
	if err != nil {
		panic(err)
	}
}
